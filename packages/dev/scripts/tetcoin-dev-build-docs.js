...
function buildTypedoc (docRoot) {
  fs
    ...
    .forEach(([full, dir]) => {
      execSync(`${path.join(__dirname, 'tetcoin-exec-typedoc.js')} --theme markdown --out ${docRoot}/${dir} ${full}/src`, { stdio: 'inherit' });
    });
}

function buildVuepress (docRoot) {
  execSync(`${path.join(__dirname, 'tetcoin-exec-vuepress.js')} build ${docRoot}`, { stdio: 'inherit' });
